...
VALUES (?, ?, ?, ?, ?, ?, ?)
RETURNING id, name, cols, "rows", theme_id, in_rotation, rotation_order, dwell_seconds, layout_json, render_mode, advance_mode, created_at, updated_at
`
...
SELECT id, name, cols, "rows", theme_id, in_rotation, rotation_order, dwell_seconds, layout_json, render_mode, advance_mode, created_at, updated_at FROM views WHERE id = ?
`
...
SELECT id, name, cols, "rows", theme_id, in_rotation, rotation_order, dwell_seconds, layout_json, render_mode, advance_mode, created_at, updated_at FROM views WHERE in_rotation = 1 ORDER BY rotation_order, id
`
...
SELECT id, name, cols, "rows", theme_id, in_rotation, rotation_order, dwell_seconds, layout_json, render_mode, advance_mode, created_at, updated_at FROM views ORDER BY rotation_order, id
`
...
const updateViewAdvance = `-- name: UpdateViewAdvance :exec
UPDATE views SET advance_mode = ?, updated_at = datetime('now') WHERE id = ?
`

type UpdateViewAdvanceParams struct {
	AdvanceMode string `json:"advance_mode"`
	ID          int64  `json:"id"`
}

func (q *Queries) UpdateViewAdvance(ctx context.Context, arg UpdateViewAdvanceParams) error {
	_, err := q.db.ExecContext(ctx, updateViewAdvance, arg.AdvanceMode, arg.ID)
	return err
}
